/*
  This is ArduMark V1.0 by @bockring. Don't copy this.
...
void loop() {
  ...
  start = 0;
  Serial.println("Welcome to ArduMark V1.0 benchmark software by @bockring."); //Short introduction
...
*/
  ...
  digitalWrite(13, LOW);
  Serial.println("Generating random numbers and devisions are done.");

  //Square root calculation starting
  digitalWrite(13, HIGH);
  for(count = 0; count < 32768; count ++){
    random1 = random(0, 65536); //Generate random number
    random2 = random1 - 1;
  ...
  digitalWrite(13, LOW);
  Serial.println("Square root calculations are done.");
  ...
  Serial.println(" ");
  Serial.println("Thank you for using ArduMark V1.0 benchmark software by @bockring");
  Serial.println(" ");
