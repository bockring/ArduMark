/*
  This is ArduMark v4 by @bockring. Don't copy this.
  ...
  Update features
  *Correct letters(Pro's'ess to Pro'c'ess)
  *Add prosess number
  *Add cool time for device
  *Add some comments to code
  *Add status lamp(On-board LED ; Pin13)
   Lamp ON :Processing
...
void loop() {
  ...
  start = 0;
  Serial.println("Welcome to ArduMark v4 benchmark software by @bockring."); //Short introduction
  ...
  for (count = 0; count < 2048; count++) {  //Repeat 2048 times
    ...
    calcpros = calcpros / 81.92; //Calculate parcent
    Serial.print (calcpros); //Progress output 2
    Serial.print("% Done ["); //Progress output3
    calcpros = calcpros / 5; //Calculate to output progress bar 1
  ...
  digitalWrite(13, LOW);
  Serial.println("Sine calculations are done.(Progress : 25% Done [=====               ])"); //Output that sine calculation is done.
...
*/
  ...
  for (count = 0; count < 2048; count++) { //Repeat 2048 times
    ...
    calcpros = calcpros / 81.92; //*2
    Serial.print(calcpros); //*1
    Serial.print("% Done ["); //*1
    calcpros = calcpros / 5; //*2
  ...
  Serial.println(" ");
  Serial.println("Thank you for using ArduMark v4 benchmark software by @bockring");
  Serial.println(" ");
